...
// Pega esta función que falta en lib/wordpressClient.js
...
async function deleteWordPressPost(postId) {
    ...
    // El parámetro 'force=true' asegura el borrado permanente
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Error al eliminar el post ${postId}: ${response.statusText} - ${JSON.stringify(errorBody)}`);
        }

        return await response.json();
    } catch (error) {
        console.error(`Error en la llamada a la API para eliminar el post ${postId}:`, error);
        throw error;
    }
}


// Se añade la nueva función a las exportacionesmodule.exports = {
module.exports = {
    publishToWordPress,
    uploadImage,
    deleteWordPressPost,
